...
Each sector has a procedural score with a generated hacker-flavoured track name: an authored, attributed folk melody over a rhythm section assembled from genre banks (drum & bass, dubstep, UK bass, house, UK garage), with the genre drawn at random per sector. Every drum and bass part is drawn from large sets of one-bar patterns, two per part, alternated and varied per bar with fills, drop-outs and swing, so arrangements never repeat exactly. The arrangement layers up with the attack number: kick and bass alone, then snare and hats, then the lead, then percussion, open hats and fills, then rolls, stabs and an echo. Clearing a sector plays a calm rendition of that sector's score (half-time drums, sustained bass, the lead an octave up with an echo, wide pads) until the next sector is connected. Style changes never cut: one of four planned transitions is drawn at random (a four-bar crossfade, a fill-then-cut on the bar, a low-pass sweep, or a two-bar breakdown to pads and bass before the drop). Instruments added by a new attack enter each in their own way: drums drop in on a bar boundary announced by a fill, hats fade or build in a stutter, the bass starts on a downbeats-only variation, the lead rises in, percussion and stabs fade. Settings offer separate music and effects faders, a reroll that redraws everything (genre, melody, tuning, key, tempo, name and patterns), a reset, and a shareable track code that reproduces the exact score when pasted.
