...
function render() {
  ...
  audio.setSector(
    profile.seed,
    (a?.level ?? profile.unlocked) - 1,
    ((a?.seed ?? 0) + (profile.settings.musicVariant ?? 0) * 7919) >>> 0,
    profile.settings.musicGenre ?? 'auto',
  );
